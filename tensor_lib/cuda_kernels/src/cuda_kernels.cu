...
void cuda_element_add_packed(size_t* mat1_ids, size_t* mat2_ids, size_t* out_mat_ids, size_t num_matrices, size_t mat_rows, size_t mat_cols, bool inplace) {
    ...
    memory_manager_upload_to_allocation(gpu_mat1_buffers_block, 0, &gpu_mat1_buffers[0], sizeof(float*) * num_matrices);
    auto gpu_mat2_buffers_block = memory_manager_allocate(sizeof(float*) * num_matrices);
    memory_manager_upload_to_allocation(gpu_mat2_buffers_block, 0, &gpu_mat2_buffers[0], sizeof(float*) * num_matrices);
    auto gpu_out_buffers_block = memory_manager_allocate(sizeof(float*) * num_matrices);
    memory_manager_upload_to_allocation(gpu_out_buffers_block, 0, &gpu_out_buffers[0], sizeof(float*) * num_matrices);

    float** gpu_mat1_buffers_dp = (float**)get_block_gpu_address(gpu_mat1_buffers_block, 0);
    float** gpu_mat2_buffers_dp = (float**)get_block_gpu_address(gpu_mat2_buffers_block, 0);
    float** gpu_out_buffers_dp = (float**)get_block_gpu_address(gpu_out_buffers_block, 0);
    ...
    memory_manager_free(gpu_mat1_buffers_block, sizeof(float*) * num_matrices);
    memory_manager_free(gpu_mat2_buffers_block, sizeof(float*) * num_matrices);
    memory_manager_free(gpu_out_buffers_block, sizeof(float*) * num_matrices);
}
...
// Each block handles one matrix
__global__ void cuda_scalar_multiply_packed_kernel(float** mat_buffers, float** out_buffers, int mat_rows, int mat_cols, float* scalars) {
    const int current_matrix = blockIdx.x;
    int tidX = threadIdx.x;
    int tidY = threadIdx.y;
    ...
    const float scalar = scalars[current_matrix];
    float* out_buffer = out_buffers[current_matrix];
...
// Each block handles one matrix
__global__ void cuda_scalar_multiply_packed_inplace_kernel(float** mat_buffers, float** out_buffers, int mat_rows, int mat_cols, float* scalars) {
    const int current_matrix = blockIdx.x;
    int tidX = threadIdx.x;
    int tidY = threadIdx.y;
...
void cuda_scalar_multiply_packed(size_t* mat_ids, size_t* out_mat_ids, size_t num_matrices, size_t mat_rows, size_t mat_cols, float* scalars, bool inplace) {
    // Create output buffer
    int out_rows = mat_rows;
    int out_cols = mat_cols;
    ...
    auto gpu_mat1_buffers_block = memory_manager_allocate(sizeof(float*) * num_matrices);
    memory_manager_upload_to_allocation(gpu_mat1_buffers_block, 0, &gpu_mat1_buffers[0], sizeof(float*) * num_matrices);
    auto gpu_scalars_block = memory_manager_allocate(sizeof(float) * num_matrices);
    memory_manager_upload_to_allocation(gpu_scalars_block, 0, &scalars[0], sizeof(float) * num_matrices);
    auto gpu_out_buffers_block = memory_manager_allocate(sizeof(float*) * num_matrices);
    memory_manager_upload_to_allocation(gpu_out_buffers_block, 0, &gpu_out_buffers[0], sizeof(float*) * num_matrices);

    float** gpu_mat1_buffers_dp = (float**)get_block_gpu_address(gpu_mat1_buffers_block, 0);
    float* gpu_scalars_buffer = (float*)get_block_gpu_address(gpu_scalars_block, 0);
    float** gpu_out_buffers_dp = (float**)get_block_gpu_address(gpu_out_buffers_block, 0);
    ...
    kernel<<<grid_dim, block_dim, 0, get_stream()>>>(gpu_mat1_buffers_dp, gpu_out_buffers_dp, mat_rows, mat_cols, gpu_scalars_buffer);
    gpuErrchk(cudaPeekAtLastError());

    // Free the gpu buffers
    memory_manager_free(gpu_mat1_buffers_block, sizeof(float*) * num_matrices);
    memory_manager_free(gpu_scalars_block, sizeof(float) * num_matrices);
    memory_manager_free(gpu_out_buffers_block, sizeof(float*) * num_matrices);
}
...
// Each block handles one matrix
__global__ void cuda_scalar_divide_packed_kernel(float** mat_buffers, float** out_buffers, int mat_rows, int mat_cols, float* scalars) {
    const int current_matrix = blockIdx.x;
    int tidX = threadIdx.x;
    int tidY = threadIdx.y;
...
// Each block handles one matrix
__global__ void cuda_scalar_divide_packed_inplace_kernel(float** mat_buffers, float** out_buffers, int mat_rows, int mat_cols, float* scalars) {
    const int current_matrix = blockIdx.x;
    int tidX = threadIdx.x;
    int tidY = threadIdx.y;
...
void cuda_scalar_divide_packed(size_t* mat_ids, size_t* out_mat_ids, size_t num_matrices, size_t mat_rows, size_t mat_cols, float* scalars, bool inplace) {
    // Create output buffer
    int out_rows = mat_rows;
    int out_cols = mat_cols;
...
// Each block handles one matrix
__global__ void cuda_scalar_add_packed_kernel(float** mat_buffers, float** out_buffers, int mat_rows, int mat_cols, float* scalars) {
    const int current_matrix = blockIdx.x;
    int tidX = threadIdx.x;
    int tidY = threadIdx.y;
...
// Each block handles one matrix
__global__ void cuda_scalar_add_packed_inplace_kernel(float** mat_buffers, float** out_buffers, int mat_rows, int mat_cols, float* scalars) {
    const int current_matrix = blockIdx.x;
    int tidX = threadIdx.x;
    int tidY = threadIdx.y;
...
void cuda_scalar_add_packed(size_t* mat_ids, size_t* out_mat_ids, size_t num_matrices, size_t mat_rows, size_t mat_cols, float* scalars, bool inplace) {
    // Create output buffer
    int out_rows = mat_rows;
    int out_cols = mat_cols;
...
// Each block handles one matrix
__global__ void cuda_scalar_subtract_packed_kernel(float** mat_buffers, float** out_buffers, int mat_rows, int mat_cols, float* scalars) {
    const int current_matrix = blockIdx.x;
    int tidX = threadIdx.x;
    int tidY = threadIdx.y;
...
// Each block handles one matrix
__global__ void cuda_scalar_subtract_packed_inplace_kernel(float** mat_buffers, float** out_buffers, int mat_rows, int mat_cols, float* scalars) {
    const int current_matrix = blockIdx.x;
    int tidX = threadIdx.x;
    int tidY = threadIdx.y;
...
void cuda_scalar_subtract_packed(size_t* mat_ids, size_t* out_mat_ids, size_t num_matrices, size_t mat_rows, size_t mat_cols, float* scalars, bool inplace) {
    // Create output buffer
    int out_rows = mat_rows;
    int out_cols = mat_cols;
...
void cuda_element_sqrt_packed(size_t* mat_ids, size_t* out_mat_ids, size_t num_matrices, size_t mat_rows, size_t mat_cols, bool inplace) {
    ...
    float** gpu_mat_buffers_dp = (float**)get_block_gpu_address(gpu_mat_buffers_block, 0);
    float** gpu_out_buffers_dp = (float**)get_block_gpu_address(gpu_out_buffers_block, 0);
    ...
    // Free the gpu array
    memory_manager_free(gpu_mat_buffers_block, sizeof(float*) * num_matrices);
    memory_manager_free(gpu_out_buffers_block, sizeof(float*) * num_matrices);
}
...
void cuda_max_pool_packed(size_t* mat_ids, Tuple* out_ids, size_t num_matrices, size_t mat_rows, size_t mat_cols) {
    ...
    memory_manager_upload_to_allocation(gpu_max_bitmasks_block, 0, &gpu_max_bitmasks[0], sizeof(float*) * num_matrices);

    float** gpu_mat_buffers_dp = (float**)get_block_gpu_address(gpu_mat_buffers_block, 0);
    float** gpu_out_buffers_dp = (float**)get_block_gpu_address(gpu_out_buffers_block, 0);
    float** gpu_max_bitmasks_dp = (float**)get_block_gpu_address(gpu_max_bitmasks_block, 0);
    ...
    memory_manager_free(gpu_mat_buffers_block, sizeof(float*) * num_matrices);
    memory_manager_free(gpu_out_buffers_block, sizeof(float*) * num_matrices);
    memory_manager_free(gpu_max_bitmasks_block, sizeof(float*) * num_matrices);
}
...
void cuda_correlate_valid_packed(size_t* matrices_ids, size_t num_matrices, size_t mat_rows, size_t mat_cols, size_t* kernel_ids, size_t kernel_rows, size_t kernel_cols, size_t* out_mat_ids) {
    ...
    auto gpu_mat_buffers_block = memory_manager_allocate(sizeof(float*) * num_matrices);
    memory_manager_upload_to_allocation(gpu_mat_buffers_block, 0, &gpu_mat_buffers[0], sizeof(float*) * num_matrices);
    auto gpu_kernel_buffers_block = memory_manager_allocate(sizeof(float*) * num_matrices);
    memory_manager_upload_to_allocation(gpu_kernel_buffers_block, 0, &gpu_kernel_buffers[0], sizeof(float*) * num_matrices);
    auto gpu_out_buffers_block = memory_manager_allocate(sizeof(float*) * num_matrices);
    memory_manager_upload_to_allocation(gpu_out_buffers_block, 0, &gpu_out_buffers[0], sizeof(float*) * num_matrices);

    float** gpu_mat_buffers_ptr = (float**)get_block_gpu_address(gpu_mat_buffers_block, 0);
    float** gpu_kernel_buffers_ptr = (float**)get_block_gpu_address(gpu_kernel_buffers_block, 0);
    float** gpu_out_buffers_ptr = (float**)get_block_gpu_address(gpu_out_buffers_block, 0);
    ...
    // Run the kernels
    cuda_correlate_kernel_packed_valid_1<<<grid_dim, block_dim, 0, get_stream()>>>(gpu_mat_buffers_ptr, num_matrices, mat_rows, mat_cols, gpu_kernel_buffers_ptr, kernel_rows, kernel_cols, gpu_out_buffers_ptr, out_rows, out_cols);
    gpuErrchk(cudaPeekAtLastError());

    // Cleanup
    memory_manager_free(gpu_mat_buffers_block, sizeof(float*) * num_matrices);
    memory_manager_free(gpu_kernel_buffers_block, sizeof(float*) * num_matrices);
    memory_manager_free(gpu_out_buffers_block, sizeof(float*) * num_matrices);
}
...
void cuda_correlate_same_packed(size_t* matrices_ids, size_t num_matrices, size_t mat_rows, size_t mat_cols, size_t* kernel_ids, size_t kernel_rows, size_t kernel_cols, size_t* out_mat_ids) {
    ...
    // Run the kernels
    cuda_correlate_kernel_packed_same_1<<<grid_dim, block_dim, 0, get_stream()>>>(gpu_mat_buffers_ptr, num_matrices, mat_rows, mat_cols, gpu_kernel_buffers_ptr, kernel_rows, kernel_cols, gpu_out_buffers_ptr, out_rows, out_cols);
    gpuErrchk(cudaPeekAtLastError());
...
void cuda_correlate_full_packed(size_t* matrices_ids, size_t num_matrices, size_t mat_rows, size_t mat_cols, size_t* kernel_ids, size_t kernel_rows, size_t kernel_cols, size_t* out_mat_ids) {
    ...
    // Run the kernels
    cuda_correlate_kernel_packed_full_1<<<grid_dim, block_dim, 0, get_stream()>>>(gpu_mat_buffers_ptr, num_matrices, mat_rows, mat_cols, gpu_kernel_buffers_ptr, kernel_rows, kernel_cols, gpu_out_buffers_ptr, out_rows, out_cols);
    gpuErrchk(cudaPeekAtLastError());
...
void cuda_convolve_valid_packed(size_t* matrices_ids, size_t num_matrices, size_t mat_rows, size_t mat_cols, size_t* kernel_ids, size_t kernel_rows, size_t kernel_cols, size_t* out_mat_ids) {
    ...
    // Run the kernels
    cuda_convolve_kernel_packed_valid_1<<<grid_dim, block_dim, 0, get_stream()>>>(gpu_mat_buffers_ptr, num_matrices, mat_rows, mat_cols, gpu_kernel_buffers_ptr, kernel_rows, kernel_cols, gpu_out_buffers_ptr, out_rows, out_cols);
    gpuErrchk(cudaPeekAtLastError());
...
void cuda_convolve_same_packed(size_t* matrices_ids, size_t num_matrices, size_t mat_rows, size_t mat_cols, size_t* kernel_ids, size_t kernel_rows, size_t kernel_cols, size_t* out_mat_ids) {
    ...
    // Run the kernels
    cuda_convolve_kernel_packed_same_1<<<grid_dim, block_dim, 0, get_stream()>>>(gpu_mat_buffers_ptr, num_matrices, mat_rows, mat_cols, gpu_kernel_buffers_ptr, kernel_rows, kernel_cols, gpu_out_buffers_ptr, out_rows, out_cols);
    gpuErrchk(cudaPeekAtLastError());
...
void cuda_convolve_full_packed(size_t* matrices_ids, size_t num_matrices, size_t mat_rows, size_t mat_cols, size_t* kernel_ids, size_t kernel_rows, size_t kernel_cols, size_t* out_mat_ids) {
    ...
    // Run the kernels
    cuda_convolve_kernel_packed_full_1<<<grid_dim, block_dim, 0, get_stream()>>>(gpu_mat_buffers_ptr, num_matrices, mat_rows, mat_cols, gpu_kernel_buffers_ptr, kernel_rows, kernel_cols, gpu_out_buffers_ptr, out_rows, out_cols);
    gpuErrchk(cudaPeekAtLastError());
...
size_t cuda_img2col_valid(size_t* mat_ids, size_t num_matrices, size_t mat_rows, size_t mat_cols, size_t kernel_rows, size_t kernel_cols) {
    ...
    for (size_t i = 0; i < num_matrices; i++) {
        mat_buffers.push_back(get_matrix_gpu_address(mat_ids[i]));
    }
...
void cuda_unflatten_array(size_t array_id, size_t arr_size, size_t mat_rows, size_t mat_cols, size_t* mat_ids) {
    ...
    for (int i = 0; i < num_matrices; i++) {
        size_t mat_id = mat_ids[i];
        gpu_mat_buffers.push_back(get_matrix_gpu_address(mat_id));
    }
